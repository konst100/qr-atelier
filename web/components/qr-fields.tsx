'use client';
import { Link2, AlignLeft, Wifi, ContactRound } from 'lucide-react';
...
export function QrFields({ draft, language, update }: Props) {
  ...
  return <Tabs value={draft.kind} onValueChange={(value) => update('kind', value as QrKind)} className="type-tabs">
    ...
    <TabsContent value="url"><div className="field"><Label htmlFor="qr-url">{t.urlLabel}</Label><Input id="qr-url" value={draft.url} onChange={(event) => update('url', event.target.value)} maxLength={1200} type="url" placeholder="https://example.com" aria-describedby="url-hint" /><p id="url-hint" className="field-hint">{t.urlHint}</p></div></TabsContent>
