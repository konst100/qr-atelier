...
type Props = { draft: QrDraft; language: Language; update: <K extends keyof QrDraft>(key: K, value: QrDraft[K]) => void };
export function QrFields({ draft, language, update }: Props) {
  const t = translations[language];
  return <Tabs value={draft.kind} onValueChange={(value) => update('kind', value as QrKind)} className="type-tabs">
    <TabsList aria-label={t.content} className="type-list">{([{ kind: 'url', Icon: Link2 }, { kind: 'text', Icon: AlignLeft }, { kind: 'wifi', Icon: Wifi }, { kind: 'contact', Icon: ContactRound }] as const).map(({ kind, Icon }) => <TabsTrigger key={kind} value={kind}><Icon size={17}/>{t[kind]}</TabsTrigger>)}</TabsList>
    <TabsContent value="url"><div className="field"><Label>{t.linkMode}</Label><div className="mode-row"><button type="button" className={draft.mode === 'static' ? 'mode-button selected' : 'mode-button'} aria-pressed={draft.mode === 'static'} onClick={() => update('mode', 'static')}><Radio size={15}/><span><strong>{t.staticMode}</strong><small>{t.staticModeHint}</small></span></button><button type="button" className={draft.mode === 'dynamic' ? 'mode-button selected' : 'mode-button'} aria-pressed={draft.mode === 'dynamic'} onClick={() => update('mode', 'dynamic')}><Zap size={15}/><span><strong>{t.dynamicMode}</strong><small>{t.dynamicModeHint}</small></span></button></div></div><div className="field"><Label htmlFor="qr-url">{t.urlLabel}</Label><Input id="qr-url" value={draft.url} onChange={(event) => update('url', event.target.value)} maxLength={1200} type="url" placeholder="https://example.com" aria-describedby="url-hint" /><p id="url-hint" className="field-hint">{t.urlHint}</p></div>{draft.mode === 'dynamic' && <div className="field"><Label htmlFor="qr-slug">{t.slug}</Label><Input id="qr-slug" value={draft.slug} onChange={(event) => update('slug', event.target.value)} maxLength={64} placeholder="summer-2026" aria-describedby="slug-hint" /><p id="slug-hint" className="field-hint">{t.slugHint}</p></div>}</TabsContent>
